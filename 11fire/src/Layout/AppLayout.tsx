  ...
  return (
    <Box sx={{ display: 'flex', height: '100vh' }}>
      ...
      <Box sx={{ width: 200, bgcolor: '#EF4444', display: 'flex', flexDirection: 'column', alignItems: 'center', pt: 4 }}>
        ...
        <List sx={{ width: '100%' }}>
          <Box sx={{ bgcolor: selectedTab === 'home' ? '#FFF7ED' : '#EF4444' }}>
            <ListItemButton selected={selectedTab === 'home'} onClick={() => setSelectedTab('home')} sx={getTabStyles('home')}>
              <ListItemIcon sx={{ color: getIconColor('home') }}><HomeRoundedIcon /></ListItemIcon>
              <ListItemText primary={<Typography sx={{ color: getTextColor('home') }}>Home</Typography>} />
            </ListItemButton>
          </Box>
